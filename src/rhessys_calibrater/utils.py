...
class rhessys_cluster:
    ...
    def __init__(self, 
        ...
        output_len = len(list(set([i.split('_')[0] for i in os.listdir(self.output_folder)])))
        ...
        if output_len != len(self.param):
            ...
            print(', '.join(list(self.param.columns[1:])))

            self.param_names = list(self.param.columns[1:])
    ...
    def KclusterSampling(self, 
                ...
                table_out=None,
                method='greater',
                sample_size=200,  # number of sampling from each cluster
        ...
        cluster_mean = temp.groupby('label').mean()
        cluster_std = temp.groupby('label').std()
